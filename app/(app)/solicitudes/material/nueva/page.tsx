...
export default function MaterialRequestNewPage() {
  ...
  async function saveDraft(): Promise<string> {
    ...
    if (!obraId) throw new Error("Selecciona una obra.");
  ...
  async function onAttach(file: File) {
    try {
      ...
      fd.set("file", file);
      const res = await fetch("/api/solicitud-files/upload", {
        method: "POST",
        credentials: "include",
        body: fd,
      });
    ...
    <div className="mx-auto max-w-3xl space-y-6">
      <header>
        ...
        <p className="mt-1 text-sm text-zinc-500">
          Elige la obra y agrega cada material con su cantidad y unidad. Estado inicial: borrador.
        </p>
      </header>
      ...
      <section className="card space-y-5 p-5 sm:p-6">
        ...
        <label className="block">
          ...
          <textarea
            ...
            rows={3}
            className={inputCls}
            placeholder="Por qué se necesita el material…"
          />
        </label>
        <div>
          ...
          <FilePickButton
            disabled={busy}
            label="Adjuntar PDF"
            hint="especificaciones o planos"
            onPick={(f) => void onAttach(f)}
          />
        </div>
      </section>
      ...
      <div className="flex flex-wrap gap-3">
        <button
          ...
          onClick={() =>
            void saveDraft()
              .then(() => showSuccess("Borrador guardado."))
