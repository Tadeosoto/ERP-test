      ...
      <div className="mt-2 overflow-x-auto rounded-xl border border-zinc-200">
        <table className="min-w-full divide-y divide-zinc-200 text-left text-sm">
          ...
          <tbody className="divide-y divide-zinc-100 bg-white">
            {files.map((f) => (
              <tr key={f.id} className="align-middle hover:bg-zinc-50/70">
                <td className="px-3 py-2">
                  <p className="font-medium text-zinc-900">{FILE_KIND_LABEL[f.kind] ?? f.kind}</p>
                  <p className="truncate text-xs text-zinc-500">{f.originalFileName}</p>
                </td>
                  ...
                  <div className="flex flex-wrap items-center justify-end gap-1.5">
                    ...
                    {canReplaceFile?.(f) && onReplace && (
                      <button
                        type="button"
                        disabled={busy}
                        className="min-h-8 rounded-lg px-2 text-xs font-semibold text-violet-800 hover:bg-violet-50 disabled:opacity-50"
                    ...
                    {canDelete && onDelete && (
                      <button
                        type="button"
                        disabled={busy}
                        className="min-h-8 rounded-lg px-2 text-xs font-semibold text-red-700 hover:bg-red-50 disabled:opacity-50"
